...
export default {
  ...
  computed: {
    ...
    clusters() {
      ...
      let kubeClusters = filterHiddenLocalCluster(
        filterOnlyKubernetesClusters(all, this.$store),
        this.$store
      );
      let pClusters = null;
    ...
    multiClusterApps() {
      ...
      return options.filter((opt) => {
        const filterApps =
          (opt.inStore === 'management' || opt.isMultiClusterApp) &&
          opt.category !== 'configuration' &&
          opt.category !== 'legacy';
    ...
    hciApps() {
      ...
      return options.filter(
        (opt) => this.isRancherInHarvester && opt.category === 'hci'
      );
    },
